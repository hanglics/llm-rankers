...
- **Primary target:** ICTIR 2026 (per `research_pipeline_setwise/RESEARCH_BRIEF.md:34`, `PAPER_PLAN.md:7`, `RESULTS_REVIEW_V1.md:20`).
...
## Open decision points (from `NARRATIVE_REPORT.md` and `RESULTS_REVIEW_V1.md`)
...
4. Same-method / same-sort result tables (per `Need_to_Run.txt` top priority) — must exist before submission; currently only best-per-model numbers are compiled.
