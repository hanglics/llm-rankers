#!/bin/bash
# Submit Study A (pool-size sweep) jobs via the MaxContext BottomUp base launcher.
#
# Usage:
#   bash experiments/run_maxcontext_bottomup_pool_sweep.sh <model> <dataset> <run_path> <output_dir> \
...
OUTPUT_DIR=${4:-"results/maxcontext_bottomup_pool_sweep/qwen3-4b-dl19"}
DEVICE=${5:-"cuda"}
...
for POOL_SIZE in 10 20 30 40 50; do
    ...
    JOB_TAG="mcbupool-${MODEL_TAG}-${DATASET_TAG}-p${POOL_SIZE}"
