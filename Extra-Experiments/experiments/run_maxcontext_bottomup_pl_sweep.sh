#!/bin/bash
# Submit Study B (passage-length sweep) jobs via the MaxContext BottomUp base launcher.
#
# Usage:
#   bash experiments/run_maxcontext_bottomup_pl_sweep.sh <model> <dataset> <run_path> <output_dir> \
...
OUTPUT_DIR=${4:-"results/maxcontext_bottomup_pl_sweep/qwen3-4b-dl19"}
DEVICE=${5:-"cuda"}
...
for PASSAGE_LENGTH in 64 128 256 512; do
    ...
    JOB_TAG="mcbupl-${MODEL_TAG}-${DATASET_TAG}-p${POOL_SIZE}-pl${PASSAGE_LENGTH}"
    echo "Submitting ${JOB_TAG}"
    sbatch -J "${JOB_TAG}" \
        -o "logs/${JOB_TAG}-%j.out" \
        -e "logs/${JOB_TAG}-%j.err" \
        experiments/run_maxcontext_bottomup.sh \
