...
> **Template for Workflow 3 (`/paper-writing`).** Fill in all sections, then run `/paper-writing "NARRATIVE.md"`.
...
We ran paired approximate-randomization tests directly on the saved per-query `ndcg_cut_10` values in the `.eval` files, with Bonferroni correction across the 18 configs per family. The full artifact is in [`SIGNIFICANCE_TESTS.md`](SIGNIFICANCE_TESTS.md).
