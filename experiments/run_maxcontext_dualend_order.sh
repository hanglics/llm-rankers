...
#       [device] [scoring] [pool_size] [passage_length] [ordering]
...
SHUFFLE_ARGS=()
if [[ "${ORDERING}" == "inverse" || "${ORDERING}" == "random" ]]; then
...
python run.py \
    run --model_name_or_path "${MODEL}" \
        ...
        --run_path "${RUN_PATH}" \
        --save_path "${OUTPUT_DIR}/maxcontext_dualend_${ORDERING}.txt" \
        --device "${DEVICE}" \
        --scoring "${SCORING}" \
        --hits "${POOL_SIZE}" \
        --query_length 128 \
    ...
    setwise --num_child 3 \
            ...
            --num_permutation 1 \
            --direction maxcontext_dualend \
    2>&1 | tee "${OUTPUT_DIR}/maxcontext_dualend_${ORDERING}.log"
